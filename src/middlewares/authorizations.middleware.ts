...
class Authorization {
  ...
  middleware = async (req: UserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!token) {
        HelpersService.sendResponse(res, 401, "No token provided");
        return;
      }
      ...
      if (error.name === "TokenExpiredError") {
        HelpersService.sendResponse(res, 401, "Token expired");
        return;
      }
...
export const authMiddleware: RequestHandler = new Authorization(
  new ProfileService()
).middleware;
